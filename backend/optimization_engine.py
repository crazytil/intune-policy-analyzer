...
def _extract_csp_area(path: str) -> str | None:
    ...
    for i, seg in enumerate(segments):
        if seg.lower() == "config" and i + 1 < len(segments):
            return segments[i + 1]
    for i, seg in enumerate(segments):
        if seg.lower() == "msft" and i + 1 < len(segments):
            return segments[i + 1]
    return None
...
def _classify_domain(setting_key: str, display_name: str) -> str:
    ...
    if prefix in ("settingsCatalog", "complianceV2", "endpointSecurity"):
        ...
        # Match whole area prefixes after the structural path, longest first.
        # Splitting on every underscore turns all ADMX areas into "admx".
        for root in ("device_vendor_msft_", "user_vendor_msft_"):
            if not path.lower().startswith(root):
                continue
            area_path = path.lower()[len(root):].removeprefix("policy_config_")
            for area in sorted(_CSP_AREA_TO_DOMAIN, key=len, reverse=True):
                if area_path.startswith(area + "_"):
                    return _CSP_AREA_TO_DOMAIN[area]
        return "Other"
...
def _audience_signature(
    ...
    """Require complete identical assignments, not merely overlapping groups."""
    signatures: set[str] = set()
    labels: set[str] = set()
    has_include = False
    for assignment in policy.assignments:
        target = assignment.get("target", {})
        kind = str(target.get("@odata.type", "")).removeprefix("#microsoft.graph.")
        group_id = str(target.get("groupId", "")).strip()
        if kind in ("groupAssignmentTarget", "exclusionGroupAssignmentTarget") and group_id:
            label = (group_name_by_id or {}).get(group_id, group_id)
            if kind == "exclusionGroupAssignmentTarget":
                label = f"Exclude {label}"
            else:
                has_include = True
        elif kind in ("allDevicesAssignmentTarget", "allLicensedUsersAssignmentTarget"):
            label = "All Devices" if kind == "allDevicesAssignmentTarget" else "All Users"
            has_include = True
        else:
            # Unknown targets are not evidence that two audiences are equivalent.
            return None
        filter_id = target.get("deviceAndAppManagementAssignmentFilterId")
        if filter_id:
            label += f" ({target.get('deviceAndAppManagementAssignmentFilterType', 'unknown')} filter {filter_id})"
        labels.add(label)
        signatures.add(_canonical({key: value for key, value in assignment.items() if key != "id"}))
    if not has_include:
        return None
    return _canonical(sorted(signatures)), "; ".join(sorted(labels))
...
def _optimization_settings(policy: Policy) -> list[dict[str, Any]]:
    entries = _extract_settings(policy)
    if policy.policy_type in (PolicyType.SETTINGS_CATALOG, PolicyType.COMPLIANCE_V2):
        instances = [setting.get("settingInstance", {}) for setting in policy.settings]
        expected_count = policy.raw.get("settingCount")
        if expected_count is not None and expected_count != len(instances):
            return []
        if len(entries) != len(instances) or any(entry["value"] is None for entry in entries):
            return []
        for entry, instance in zip(entries, instances):
            # Compare the complete instance, including nested choice children.
            # Keep the shared extractor's canonical keys and display labels.
    ...
    for entry in entries:
        ...
        if entry["value"] is None:
            return []
        if key in by_key and _canonical(by_key[key]["value"]) != _canonical(entry["value"]):
            return []
        by_key[key] = entry
...
def _finding_id(
    ...
    identity = [recommendation_type.value, domain, audience_key, platform_key,
                sorted((policy.policy_type.value, policy.id) for policy in policies)]
...
def _make_finding(
    *,
    ...
    title = (
        f"Review {domain} consolidation"
        if recommendation_type == OptimizationRecommendationType.CONSOLIDATION_CANDIDATE
        else f"Review {domain} fragmentation"
    )
    summary = (
        f"{len(policies)} policies have identical assignments on {', '.join(_platform_labels(platform_key))}."
    )
    rationale = (
        f"{shared_setting_count} shared settings and {unique_setting_count} distinct settings in this domain; "
        f"{conflict_count} conflicting settings across these policies. "
        "Review complete settings and assignment intent before making changes. "
        "Analysis covers fetched data only; scores are heuristic rankings, not safety guarantees."
    )

    return OptimizationFindingV1(
        finding_id=_finding_id(recommendation_type, domain, audience_key, platform_key, policies),
...
def _iter_domain_clusters(
    ...
    for policy in policies:
        # Other families require additional payload or audience semantics that
        # the current fetcher/extractor does not preserve for consolidation.
        if policy.policy_type not in (
            PolicyType.DEVICE_CONFIGURATION, PolicyType.COMPLIANCE,
            PolicyType.SETTINGS_CATALOG, PolicyType.COMPLIANCE_V2, PolicyType.ENDPOINT_SECURITY,
        ) or "unknown" in _platform_tokens(policy):
            continue
        audience = _audience_signature(policy, group_name_by_id)
        if audience is None:
            continue
        audience_key, audience_label = audience
        family_key = _canonical([
            policy.policy_type.value, policy.raw.get("@odata.type"),
            policy.raw.get("templateReference"), policy.raw.get("templateId"),
            policy.raw.get("technologies"),
        ])
        policy_entries = _optimization_settings(policy)
        ...
        for domain, entries in by_domain.items():
            cluster_key = (domain, audience_key, _platform_bucket_key(policy), family_key)
            cluster = clusters.setdefault(cluster_key, {"audience": audience_label, "policies": {}})
            cluster["policies"][policy.id] = {
                "policy": policy,
                "settings": entries,
                "all_settings": policy_entries,
            }
...
def analyze_optimization_opportunities(
    ...
    if selected_group_id:
        # Direct include targets only. Do not imply effective group membership
        # or expand All Users/Devices without resolving exclusions and filters.
        filtered_policies = [policy for policy in filtered_policies
                             if selected_group_id in _get_assigned_group_ids(policy)]
    clusters = _iter_domain_clusters(filtered_policies, group_name_by_id)
    findings: list[OptimizationFindingV1] = []

    for (domain, audience_key, platform_key, _family_key), cluster in clusters.items():
        if domain == "Other":
            continue
        ...
        # Reuse the conflict analyzer's legacy-default rule without discarding
        # false values that differ from another policy's explicit true value.
        settings_map = {key: entries for key, entries in settings_map.items()
                        if not _should_skip_matching_entries(entries)}
        policy_settings = {key: [entry for entry in entries if entry["setting_key"] in settings_map]
                           for key, entries in policy_settings.items()}
        policies_in_cluster = [policy for policy in policies_in_cluster if policy_settings[policy.id]]
        if len(policies_in_cluster) < 2:
            continue

        all_settings: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for policy in policies_in_cluster:
            item = policy_map[policy.id]
            for entry in item["all_settings"]:
                all_settings[entry["setting_key"]].append(entry)
        conflict_count = sum(len({_canonical(entry["value"]) for entry in entries}) > 1
                             for entries in all_settings.values())
        ...
        for entries in settings_map.values():
            ...
            distinct_values = {_canonical(entry["value"]) for entry in entries}
